#define pin473LED1 2 
#define pin473Laser1 4
#define pin473LED2 8
#define pin473Laser2 12
#define pinIDLE1 7
#define pinIDLE2 13
...
void loop()
{
  ...
  {
    case 0: // idle mode
    Serial.println("Input all parameters: e.g. p473,20,6,4,20,20,3,18,1,12,4,12,");
...
//   Serial.println("Input total duration (sec):");
    ...
    time0 = millis();
    for (i = imemory; i < (long(expduration) * long(pulsefreq)); i++) // total number of pulses
    {
      //Serial.println(i);
      time2 = millis();
      ...
      if(!LEDon2 && i >= start2*pulsefreq && (i-start2*pulsefreq) % (cycleduration*pulsefreq) < dutylength * pulsefreq && i < stop2*pulsefreq){
        ...
        LEDon2 = 1; 
        Serial.println("LED2 is on at"); time1 = millis(); Serial.println(time1-time0);
      ...
      time3 = millis();
      Serial.println(max(0,1000/long(pulsefreq) - (time3 - time2)));
      delay(max(0,1000/long(pulsefreq) - (time3 - time2)));
    }
    if (Mode == 4){break;} 
    else if (Mode == 0) {
      imemory = 0; 
